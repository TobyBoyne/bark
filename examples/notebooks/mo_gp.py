...
train_y1 = torch.sin(train_x1 * (2 * torch.pi)) + torch.randn(train_x1.size()) * 1.0
train_y2 = torch.sin(train_x2 * (2 * torch.pi)) + torch.randn(train_x2.size()) * 0.1
...
likelihood = MultitaskGaussianLikelihood(num_tasks=2, train_task_idxs=full_train_i)
...
with torch.no_grad(), gpy.settings.fast_pred_var():
    observed_pred_y1 = likelihood(model(test_x, test_i_task1), task_idxs=test_i_task1)
    observed_pred_y2 = likelihood(model(test_x, test_i_task2), task_idxs=test_i_task2)
