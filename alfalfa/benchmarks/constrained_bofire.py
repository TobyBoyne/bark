...
from alfalfa.bofire_utils.constraints import (
...
class G4(Benchmark):
    ...
    def __init__(self, **kwargs):
        ...
        self._domain = Domain(
            ...
            constraints=Constraints(
                constraints=[
                    FunctionalEqualityConstraint(func=lambda x, _=None: -u(x), rhs=0.0),
                    FunctionalEqualityConstraint(func=lambda x, _=None: u(x), rhs=92.0),
                    FunctionalEqualityConstraint(
                    ...
                    FunctionalEqualityConstraint(func=lambda x, _=None: w(x), rhs=25.0),
                ]
            ),
        )
    ...
    def __init__(self, **kwargs):
        ...
        self._domain = Domain(
            ...
            constraints=Constraints(
                constraints=[
                    FunctionalInequalityConstraint(
                        func=lambda x, _: -((x[0] - 5) ** 2) - (x[1] - 5) ** 2,
                        rhs=-100.0,
                    ),
                    FunctionalInequalityConstraint(
                        func=lambda x, _: (x[0] - 6) ** 2 + (x[1] - 5) ** 2, rhs=82.81
                    ),
                ]
            ),
        )
...
class Alkylation(Benchmark):
    ...
    def __init__(self, **kwargs):
        ...
        def _setup_aux_vars(x, model_core: gurobipy.Model):
