...
def soft_forest_predict(
    ...
    # TODO: combine this with forest_predict
    # flatten model
    bark_model = bark_model.get_flattened_samples()
    num_samples = bark_model.noise_var.shape[0]
    num_test = test_X.shape[0]
    ...
    forest_gram_matrix = jax.jit(
        jax.vmap(soft_forest.soft_forest_gram_matrix, in_axes=(None, 0, None))
    )
    forest_covar_matrix = jax.jit(
        jax.vmap(soft_forest.soft_forest_covar_matrix, in_axes=(None, None, 0, None))
    )
