...
class CCOBench(Benchmark):
    ...
    def __init__(self, n_int_values: int = 6, **kwargs):
        ...
        self._domain = Domain(
            inputs=Inputs(
                features=[
                    ...
                    *[
                        build_integer_input(
                            key=f"downtilt_{i}", bounds=[0, n_int_values - 1]
                        )
                        for i in range(15)
                    ],
                    # Transmission powers (continuous)
                    *[
                        ContinuousInput(key=f"power_{i}", bounds=(30.0, 50.0))
                        for i in range(15)
                    ],
                ]
            ),
            outputs=Outputs(
                features=[
                    ContinuousOutput(
                        key="balanced_objective", objective=MinimizeObjective()
                    )
                ]
            ),
        )
    ...
    def _f(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logging.info(
            f"Evaluating {X.shape[0]} configurations on {self.evaluations}th evaluation"
        )
        self.evaluations += 1
        # Convert to numpy array and reshape
        downtilt_keys = [
            key for key in self.domain.inputs.get_keys() if key.startswith("downtilt_")
        ]
        power_keys = [
            key for key in self.domain.inputs.get_keys() if key.startswith("power_")
        ]
        ...
        for downtilts, tx_powers in zip(downtilts, tx_powers):
            ...
            (
                ...
                interference_map,
                _,
            ) = self.simulated_rsrp.get_RSRP_and_interference_powermap(
            ...
            objectives.append(
                [
                    0.5 * weak_coverage + 0.5 * over_coverage,
                ]
            )

        return pd.DataFrame(data=objectives, columns=self.domain.outputs.get_keys())
