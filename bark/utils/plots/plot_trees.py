...
def plot_tree(tree: BARKTree, ax: plt.Axes, xy: Optional[np.ndarray] = None):
    def plot_node(node: BARKNode, xy: np.ndarray):
...
def plot_forest(forest: BARKForest, ax: plt.Axes):
    xy = np.array([0.0, 0.0])
