...
class SimulatedRSRP:
    ...
    def get_RSRP_and_interference_powermap(
        self, downtilts: np.ndarray, tx_powers: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Returns RSRP and interference power for all the locations in the map.

        `downtilts` contains downtilts for all the sectors.
        `tx_powers` contains transmit powers for all the sectors.
        ...
        if (
            max(tx_powers) > self.max_TX_power_dBm
            or min(tx_powers) < self.min_TX_power_dBm
        ):
            raise ValueError("Transmit Power is out of the range")
        ...
        power_matrices = np.stack(
            [
                ...
                + tx_powers[flattened_sector_idx]
                for flattened_sector_idx, downtilt in enumerate(downtilts)
            ],
            -1,
        )
