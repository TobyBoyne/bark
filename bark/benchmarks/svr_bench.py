...
from bofire.data_models.features.api import ContinuousInput, CategoricalInput, ContinuousOutput
...
class SVRBench(Benchmark):
    def __init__(
        ...
        self._domain = Domain(
            inputs=Inputs(
                features=[
                    ...
                    *[build_integer_input(key=f"feature_{i + 1}", bounds=[0, 1]) for i in range(num_features_to_optimize)]
                ]
            ),
            outputs=Outputs(features=[ContinuousOutput(key="y", objective=MinimizeObjective())])
        )
        ...
        logging.info(f"Optimizing {self.num_features_to_optimize} features from {self.num_features_to_keep} features")
        
    def prepare_x_y(self, num_samples: int = 10000) -> Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray], List[np.ndarray]]:
        if getattr(self, "x_trains", None) is not None:
            return self.x_trains, self.x_tests, self.y_trains, self.y_tests

        dataset_path = pathlib.Path(__file__).parent / "data" / "slice_localization_data.csv"
        ...
        feature_select_inds = np.argsort(feature_select_regr.feature_importances_)[::-1][:self.num_features_to_keep]  # Keep 50 features
    ...
    def _f(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(X)):
            ...
            for j in range(5):
                ...
                features_filter = np.array(
                    [int(getattr(svm_hyp, f"feature_{j + 1}")) for j in range(self.num_features_to_optimize)]
        ...
        self.evaluations += 1
        return pd.DataFrame(data=np.array(evaluations).reshape(-1, 1), columns=self.domain.outputs.get_keys())
