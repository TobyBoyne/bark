...
from bark.utils.domain import get_feature_by_index
from bark.utils.logger import Timer
...
class BARKNode:
    ...
    def __init__(self):
        ...
        self.tree: Optional[BARKTree] = None
    ...
    def replace_self(self, new_node: "BARKNode"):
...
class DecisionNode(BARKNode):
    def __init__(
        self,
        var_idx: Optional[int] = None,
        threshold: Optional[float | list[int]] = None,
        left: Optional["BARKNode"] = None,
        right: Optional["BARKNode"] = None,
    ):
        super().__init__()
        self.var_idx = var_idx
        self.threshold = threshold

        self._left: BARKNode = None
        self._right: BARKNode = None
    ...
    def _set_child_data(self, child: BARKNode, recurse=True):
...
class BARKTree:
    def __init__(self, height=0, root: Optional[BARKNode] = None):
        if root is not None:
            self.root = root
        else:
    ...
    def _get_nodes_by_depth(self) -> dict[int, list[BARKNode]]:
        nodes = [self.root]
        nodes_by_depth = {}
        depth = 0
        while nodes:
    ...
    def structure_eq(self, other: "BARKTree"):
...
class BARKForest:
    def __init__(
        ...
        height=None,
        num_trees=None,
        trees: Optional[list[BARKTree]] = None,
        frozen: bool = False,
    ):
        self.trees: Sequence[BARKTree]
        if trees:
            self.trees = trees
        else:
            self.trees = [BARKTree(height) for _ in range(num_trees)]
    ...
    def structure_eq(self, other: "BARKForest"):
        return all(
    ...
    @classmethod
    def from_dict(cls, d):
        trees = [BARKTree.from_dict(d_tree) for d_tree in d["trees"]]
